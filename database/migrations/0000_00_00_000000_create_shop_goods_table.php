...
class CreateShopGoodsTable extends Migration {
	...
	public function up()
	{
		Schema::create('shop_goods', function(Blueprint $table)
		{
			...
			$table->decimal('price', 20)->nullable()->comment('销售价格-sku最低价');
			$table->integer('sku_id')->nullable()->comment('最低价对应的SKU');
			$table->integer('sku_num')->nullable()->comment('SKU数量');
			$table->decimal('max_price', 20)->nullable()->default(0.00)->comment('最高价格');
			...
			$table->integer('unit_id')->nullable()->comment('单位');
			$table->integer('logistics_id')->nullable()->comment('物流');
