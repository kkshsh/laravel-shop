...
class CreateShopGoodsProductTable extends Migration {
	...
	public function up()
	{
		Schema::create('shop_goods_product', function(Blueprint $table)
		{
			...
			$table->decimal('price', 20)->nullable()->index('idx_price')->comment('销售价格');
			$table->decimal('market_price', 20)->nullable()->comment('市场价格');
			$table->decimal('cost_price', 20)->nullable()->comment('成本价');
			$table->integer('weight')->nullable()->default(0)->comment('重量');
